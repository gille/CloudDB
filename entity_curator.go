...
func curatorEntityRootKey(c context.Context) *datastore.Key {
	return DB.NewKey(c, curatorDBEntity, curatorDBEntityRootKey, 0, nil)
}
...
func insertCurator(request *restful.Request, response *restful.Response) {
	...
	// and now store it
	key := DB.NewIncompleteKey(ctx, curatorDBEntity, curatorEntityRootKey(ctx))
	key, err := DB.Put(ctx, key, curatorDB)
	if err != nil {
...
func getCurator(request *restful.Request, response *restful.Response) {
	...
	var q Query
	if curatorString != "" {
		q = DB.NewQuery(curatorDBEntity).Filter("CuratorId =", curatorString)
	} else {
		q = DB.NewQuery(curatorDBEntity)
